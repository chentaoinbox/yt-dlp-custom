...
'''
    ...
    def main(self):
        ...
        if not self.checkconfig():
            ...
            # print(f"ffmpeginfo: {ffmpeginfo}")
            # print(f"nodejsinfo: {nodejsinfo}")
            # 查找包含目标内容的行
            target_index = None
            for i, line in enumerate(lines):
                if nodejsinfo in line:  # 已经存在 Node.js 配置
                    target_index = None
                    break
                if "# --remote-components COMPONENT" in line:  # 目标内容
                    target_index = i
                    break

            if target_index is not None:
                insert_index = target_index - 2
                if insert_index < 0:
                    insert_index = 0
                lines.insert(insert_index + 1, f"{nodejsinfo}\n")

            target_index = None
            for i, line in enumerate(lines):
                if ffmpeginfo in line:  # 已经存在 FFmpeg 配置
                    target_index = None
                    break
                if "# --exec [WHEN:]CMD" in line:  # 目标内容
                    target_index = i
                    break
            ...
            if target_index is not None:
                ...
                if insert_index < 0:
                    insert_index = 0
                lines.insert(insert_index, f"{ffmpeginfo}\n")
